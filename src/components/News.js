...
const News =(props)=> {
    ...
    const  updatenews =async ()=> {
        console.log("I AM UNDER Update news");
    ...
    const fetchMoreData = async () => {
        ...
        if (data.ok) {
            ...
            console.error('Error: Unable to fetch articles from the API');
            setLoading(false);
            // Return a message to display when API call fails
            return <div>Unable to Fetch Articles from the API</div>;
        }
    };
                ...
                <InfiniteScroll
                    ...
                    <div className="row" >
                        {articles.map((element, index) => (
                            <div className="col-md-3" key={`${element.url}-${index}`}  style={{color: props.mode==='dark'?'white':'rgb(121 77 177)'}}>
                                <Newsitem
